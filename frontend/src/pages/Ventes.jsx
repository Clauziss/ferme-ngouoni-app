...
export default function Ventes() {
  // Vente dont on consulte/complète les versements, et compteur servant à
  // forcer le rechargement du tableau après un encaissement.
  const [ventePaiement, setVentePaiement] = useState(null);
  const [rechargerTableau, setRechargerTableau] = useState(null);
  ...
  return (
    ...
    <EntityPage
      ...
      actionLigne={(vente, recharger) =>
        reste(vente) > 0 ? (
          <button
            className="bouton secondaire"
            style={{ padding: "4px 10px", fontSize: "0.78rem" }}
            onClick={() => {
              setVentePaiement(vente.id);
              setRechargerTableau(() => recharger);
            }}
            title="Enregistrer un versement complémentaire"
          >
            💵 Encaisser
          </button>
        ) : (
          <span className="puce ok">Soldée</span>
        )
      }
    />
    ...
    {ventePaiement && (
      <DialoguePaiement
        venteId={ventePaiement}
        onFermer={() => setVentePaiement(null)}
        onEnregistre={() => { if (rechargerTableau) rechargerTableau(); }}
      />
    )}
    </>
  );
