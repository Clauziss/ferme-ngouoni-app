...
const { Utilisateur, Lot, ProductionOeuf, ProductionChair, Depense, Vente, Abonne, Vaccination } = require("../models");
...
async function semerSiVide() {
  ...
  // Exemple de vaccination sur la bande de poulets de chair.
  const bandeChair = await Lot.findOne({ where: { espece: "Poulets de chair" } });
  if (bandeChair) {
    await Vaccination.create({
      date: "2026-06-08",
      lotId: bandeChair.id,
      nomVaccin: "Gumboro intermédiaire",
      maladie: "Maladie de Gumboro",
      modeAdministration: "Eau de boisson",
      nombreSujets: 198,
      dose: "1 flacon / 200 sujets",
      coutFcfa: 12000,
      dateRappel: "2026-06-22",
      observations: "Eau retirée 2 h avant l'administration",
    });
  }
  ...
  await ProductionChair.create({
    ...
    remarque: "Lot homogène, trois sujets nettement en retard de croissance.",
    appreciation: "Croissance satisfaisante pour l'âge.",
  });
